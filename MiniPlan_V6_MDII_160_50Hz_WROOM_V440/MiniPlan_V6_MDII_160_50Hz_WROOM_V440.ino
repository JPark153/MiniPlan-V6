...
#define SCREEN_MAX_TEXT_POS (24)  // Text Y position in pixels, 0, 8, 16, 24
//  I2C Address 0x3C
...
// ES08MDII Pulse Traveling 700us ~ 2300us 160 degree
...
/*============================================================================*/
// Left single-leg balance action
...
void Get_PWM_Frequency()
{
  ...
  // check boundary and clear if out of boundary
...
void setup(void) {
  ...
  // Get_PWM_Frequency();  // Get calibrate PWM frequency
  pwm.setPWMFreq(Set_PWM_Freq);
  delay(10);
  ...
  // Clear backup of current motor values
  for ( int Index = 0; Index < ALLMATRIX; Index++)
  {
    Running_Servo_POS[Index] = Servo_Act_1[Index] + (int8_t)EEPROM.read(Index);
  }
...
void loop(void) {
  ...
  if (Servo_PROGRAM >= 1 && Input_Voltage_Low == 0 )
  {
    ...
    if (OLED_Display_Index > SCREEN_MAX_TEXT_POS) {
    ...
    switch (Servo_PROGRAM)
    {
      ...
      case 4:  // Turn right
        ...
        Servo_PROGRAM_Center();
        break;
      case 5:  // Left lateral move
        Servo_PROGRAM_Run(Servo_Prg_5, Servo_Prg_5_Step);
        Servo_PROGRAM_Center();
        break;
      case 6:  // Right lateral move
  ...
  if (Servo_PROGRAM_Stack >= 1 && Input_Voltage_Low == 0 )
  {
    ...
    switch (Servo_PROGRAM_Stack)
    {
      ...
      case 9:  // Push-up action
        ...
        Servo_PROGRAM_Center();
        break;
      case 10: // TaiChi action
        Servo_PROGRAM_Run(Servo_Prg_40, Servo_Prg_40_Step);
        Servo_PROGRAM_Center();
        break;
      case 99:  // AUTO DEMO (several actions)
        Servo_PROGRAM_Run(Servo_Prg_32, Servo_Prg_32_Step);
        Servo_PROGRAM_Center();
        delay(500);
        Servo_PROGRAM_Run(Servo_Prg_36, Servo_Prg_36_Step);
